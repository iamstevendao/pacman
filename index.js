(function () {
	...
	function whereCantGo (obj) {
		let direction = []
		if (isCrashed(obj.x + 1, obj.y)) { direction.push(DIRECTION.RIGHT) }
		if (isCrashed(obj.x - 1, obj.y)) { direction.push(DIRECTION.LEFT) }
		if (isCrashed(obj.x, obj.y - 1)) { direction.push(DIRECTION.UP) }
		if (isCrashed(obj.x, obj.y + 1)) { direction.push(DIRECTION.DOWN) }
		return direction
	}
